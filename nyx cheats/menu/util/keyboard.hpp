...
{
public:
	static void on_keyboard_message(DWORD key, WORD repeats, uint8_t scanCode, BOOL isExtended, BOOL isWithAlt, BOOL wasDownBefore, BOOL isUpNow);
	void update();
	bool key_down(DWORD key);
	bool key_just_up(DWORD key, bool exclusive = true);
	bool key_just_down(DWORD button);
	int get_last_key() { return m_lastKey; }
